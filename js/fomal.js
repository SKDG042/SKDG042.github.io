...
function fetchLocationData() {
  // 使用搜狐IP接口获取IP，这个接口在国内可靠且无跨域问题
  $.ajax({
    type: "get",
    url: "https://pv.sohu.com/cityjson?ie=utf-8",
    dataType: "script",
    success: function () {
      // 搜狐API会返回一个名为returnCitySN的全局变量
      try {
        let userIp = returnCitySN.cip;
        console.log("搜狐IP获取成功:", userIp);
        ...
        $.ajax({
          ...
          data: {
            key: "b1e9effb2d59fc94c2b19a1c73fc7ed2", // 用户提供的高德地图API密钥
            ip: userIp, // 可选参数，不传则自动获取请求端IP
          },
          dataType: "json",
          ...
          error: function (err) {
            ...
            ipLoacation = {
              ip: userIp, // 使用实际获取到的IP
              country: "中国",
              province: returnCitySN.cname || "未知省份",
              city: "",
              district: "",
              longitude: 0,
              latitude: 0,
        ...
        console.log("搜狐IP数据解析失败:", e);
        fallbackToDirectAmap();
      }
    },
    error: function (err) {
      console.log("搜狐IP获取失败:", err);
      fallbackToDirectAmap();
    },
  });
}
...
function showWelcome() {
  try {
    ...
    if (ipLoacation) {
      // 计算距离
      let dist = 0;
      if (ipLoacation.latitude && ipLoacation.longitude) {
        dist = getDistance(
          106.6635156, // 东经106°36′22.856″转换为十进制度
          29.52162114, // 北纬29°31′17.836″转换为十进制度
          parseFloat(ipLoacation.longitude || 0),
          parseFloat(ipLoacation.latitude || 0)
        );
      }
      ...
      // 显示包含位置信息的欢迎消息
      welcomeInfoElement.innerHTML = `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;欢迎来自 <span style="color:var(--theme-color)">${pos}</span> 的小伙伴，${timeChange}您现在距离站长约 <span style="color:var(--theme-color)">${dist}</span> 公里，当前的IP地址为： <span style="color:var(--theme-color)">${ip}</span>， ${posdesc}</b>`;
    } else {
      // API请求失败或数据不完整，显示默认欢迎信息
      welcomeInfoElement.innerHTML = `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;欢迎访问本站！${timeChange}很高兴遇见你~</b>`;
    }
  } catch (err) {
    console.log("欢迎信息显示出错：", err);
    // 出错时也尝试显示基本欢迎信息
    try {
      document.getElementById(
        "welcome-info"
      ).innerHTML = `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;欢迎访问本站！很高兴遇见你~</b>`;
    } catch (e) {
      console.log("无法显示欢迎信息：", e);
    }
  }
}
...
function getWeibo() {
  fetch("https://v2.xxapi.cn/api/weibohot")
    .then((data) => data.json())
    .then((data) => {
      // 这里要写上你的API!!!
      let html =
  ...
  (function ($) {
    $.fn.nekoScroll = function (option) {
      var defaultSetting = {
        ...
        borderRadius: 5 + "px",
        right: 55.6 + "px",
        nekoImg: "https://i.postimg.cc/W16407LS/1743399625110-0028.jpg",
        hoverMsg: "爱你呦♪",
      ...
      $("#" + setting.nekoname).css({
        position: "fixed",
        top:
          basicInfo.Band_H * setting.zoom * basicInfo.ViewH * 0.01 - 50 + "px",
      ...
      $(window).scroll(function () {
        ...
        $(getThis).css({
          ...
          top: setting.top,
          height:
            basicInfo.Band_H * setting.zoom * basicInfo.ViewH * 0.01 + "px",
          "z-index": setting.z_index,
          "background-color": setting.bgcolor,
          "border-radius": setting.borderRadius,
          right: setting.right,
        ...
        $("#" + setting.nekoname).css({
          position: "fixed",
          top:
            basicInfo.Band_H * setting.zoom * basicInfo.ViewH * 0.01 -
            50 +
            "px",
          "z-index": setting.z_index * 10,
          right: setting.right,
...
function createtime1() {
  var grt = new Date("04/06/2025 20:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by _042 %c 你正在访问_042の小破站",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| _042")
...
var sTermInfo = [
  ...
  "7f0e37f0e366a449801e9808297c35",
  "665f67f0e37f14898082b072297c35",
  "7ec967f0e37f14998082b0787b06bd",
...
if (m == 5 && dd == 22) {
  //小猫咪生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝bro" + (y - 2008).toString() + "岁生日快乐！❤");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
...
// 切换主题提醒
if (y == 2025 && m == 4 && dd >= 18 && dd <= 20) {
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("网站换成春日限定主题啦🌸");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("04/06/2025 20:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://lskypro.acozycotage.net/_042/badge/F小屋-科研摸鱼中.svg' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://lskypro.acozycotage.net/_042/badge/F小屋-下班休息啦.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("rs") == "block") {
  document.getElementById(
    "rightSide"
  ).innerText = `:root{--rightside-display: block}`;
} else {
  document.getElementById(
    "rightSide"
  ).innerText = `:root{--rightside-display: none}`;
}
...
document.getElementById(
  "transPercent"
).innerText = `:root{--trans-light: rgba(253, 253, 253, ${curTransNum}%) !important; --trans-dark: rgba(25, 25, 25, ${curTransNum}%) !important} `;
function setTrans() {
  ...
  document.getElementById(
    "transPercent"
  ).innerText = `:root{--trans-light: rgba(253, 253, 253, ${newTransNum}%) !important; --trans-dark: rgba(25, 25, 25, ${newTransNum}%) !important} `;
}
...
document.getElementById(
  "blurNum"
).innerText = `:root{--blur-num: blur(${curBlur}px) saturate(120%) !important`;
function setBlurNum() {
...
if (localStorage.getItem("blur") == 0) {
  document.getElementById(
    "settingStyle"
  ).innerText = `:root{--backdrop-filter: none}`;
} else {
  document.getElementById(
    "settingStyle"
  ).innerText = `:root{--backdrop-filter: var(--blur-num)}`;
}
function setBlur() {
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://i.postimg.cc/QtSVZMN7/bg-1.png);
    --darkmode-bg:url(https://i.postimg.cc/RCsqPqD5/dark-bg.jpg);
    --mobileday-bg: url(https://i.postimg.cc/kgFp92KB/mobie-bg-1.png);
    --mobilenight-bg: url(https://i.postimg.cc/KY0pkC9b/dark-mobie-bg.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
...
<details class="folding-tag" cyan><summary> 查看二次元背景 </summary>
              <div class='content'>
              <div class="bgbox"><a href="javascript:;" rel="noopener external nofollow" style="background-image:url(https://lskypro.acozycotage.net/_042/img/home_bg.webp)" class="imgbox" onclick="changeBg('url(https://lskypro.acozycotage.net/_042/img/home_bg.webp)')"></a></div>
              </div>
            </details>
...
<details class="folding-tag" cyan><summary> 查看风景背景 </summary>
              <div class='content'>
              <div class="bgbox"><a href="javascript:;" rel="noopener external nofollow" style="background-image:url(https://lskypro.acozycotage.net/_042/img/fj1.webp)" class="imgbox" onclick="changeBg('url(https://lskypro.acozycotage.net/_042/img/fj1.webp)')"></a></div>
              </div>
            </details>
...
<details class="folding-tag" cyan><summary> 查看萌宠背景 </summary>
              <div class='content'>
              <div class="bgbox"><a href="javascript:;" rel="noopener external nofollow" style="background-image:url(https://lskypro.acozycotage.net/_042/img/mc1.webp)" class="imgbox" onclick="changeBg('url(https://lskypro.acozycotage.net/_042/img/mc1.webp)')"></a></div>
              </div>
            </details>
...
<details class="folding-tag" cyan><summary> 查看适配手机的背景 </summary>
              <div class='content'>
              <div class="bgbox"><a href="javascript:;" rel="noopener external nofollow" style="background-image:url(https://lskypro.acozycotage.net/_042/img/mb4.webp)" class="pimgbox" onclick="changeBg('url(https://lskypro.acozycotage.net/_042/img/mb4.webp)')"></a></div>
              </div>
            </details>
